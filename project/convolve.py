...
class Convolve():
    ...
    def _pad_matrix(m, kernel):
        ...
        (row_pad_count_top, row_pad_count_bottom), (
            col_pad_count_left, col_pad_count_right) = Convolve._calculate_padding(kernel)
    ...
    def _convolve2d(m, kernel, strides=(1, 1)):
        ...
        ValueError
            If kernel size is greater than m in any axis after padding
        """
        (target_height, target_width) = Convolve._calculate_target_matrix_dimension(m, kernel, (0, 0), strides)
        row_stride = strides[0]
        col_stride = strides[1]
        kernel_height = kernel.shape[0]
        kernel_width = kernel.shape[1]
    ...
    def convolve_volume(m_volume, kernel_volume, strides=(1, 1), use_padding=True):
        ...
        if use_padding:
            (row_pad_count_top, row_pad_count_bottom), (
                col_pad_count_left, col_pad_count_right) = Convolve._calculate_padding(kernel_volume[0])
        ...
        (target_height, target_width) = Convolve._calculate_target_matrix_dimension(m_volume[0], kernel_volume[0],
